...
namespace DIARS.Controllers
{
    ...
    [ApiController]
    public class NotaSalidaRepuestoController : ControllerBase
    {
        private readonly NotaSalidaRepuestoService _detalleService;
        private readonly IUsuarioService _usuarioService;
        private readonly IValidator<NSRListaDto> _validatorLista;
        private readonly IValidator<NSRAgregaDto> _validatorAgregar;
        private readonly IJwtService _jwtService;

        public NotaSalidaRepuestoController(NotaSalidaRepuestoService detalleService, IUsuarioService usuarioService, IValidator<NSRListaDto> validatorLista, IValidator<NSRAgregaDto> validatorAgregar, IJwtService jwtService)
        {
        ...
        [HttpGet]
        public IActionResult ListarNotaSalidaRepuesto()
        {
            ...
            try
            {
                var lista = _detalleService.ListarNotaSalidaRepuesto();

                if (lista == null || lista.Count == 0)
                    return NotFound(new { mensaje = "No hay datos disponibles." });

                return Ok(new ResponseDto<List<NSRListaDto>>
                {
        ...
        [HttpPost]
        public IActionResult CrearNotaSalidaRepuesto([FromBody] NSRAgregaDto dto)
        {
            var identity = HttpContext.User.Identity as ClaimsIdentity;
            var rToken = _jwtService.validarToken(identity, _usuarioService);
            ...
            var resultado = _detalleService.InsertarNotaSalidaRepuestos(dto);
        ...
        [HttpGet("{id}")]
        public IActionResult ObtenerNotaSalidaRepuesto(int id)
        {
            var identity = HttpContext.User.Identity as ClaimsIdentity;
            var rToken = _jwtService.validarToken(identity, _usuarioService);
            ...
            var detalle = _detalleService.GetNotaSalidaRepuestosId(id);
            ...
            return Ok(new ResponseDto<NSRListaDto>
            {
                EjecucionExitosa = true,
                MensajeError = null,
                Data = detalle
        ...
        public IActionResult EliminarPersona(int id)
        {
            ...
            try
            {
                bool resultado = _detalleService.InhabilitarNotaSalida(id);
